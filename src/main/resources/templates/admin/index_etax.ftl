	...
	<body>
								...
								<div class="row ">
                                    <div class="col-xs-12">
                                        ...
                                        <div class="form-group">
                                            文件名：<input id="targetFileName" name="targetFileName" type="text"/>
                                        </div>
		...
		<script type="text/javascript">
            $(document).ready(function () {
                //初始化表格,动态从服务器加载数据
                $("#etaxListTable").bootstrapTable({
                    //使用get请求到服务器获取数据
                    method: "GET",
                    //必须设置，不然request.getParameter获取不到请求参数
                    contentType: "application/x-www-form-urlencoded",
                    //获取数据的Servlet地址
                    url: "${ctx}/api/v1/etaxExcelFile/list",
                    //表格显示条纹
                    striped: true,
                    //启动分页
                    pagination: true,
                    ...
                    columns: [{
                        ...
                        field: "empty",
                        formatter: function (value, row, index) {
                            var operateHtml = '<button class="btn btn-primary btn-xs" type="button" onclick="view(\''+row.fileId+'\')"><i class="fa fa-edit"></i>&nbsp;预览</button> &nbsp;';
                            operateHtml = operateHtml + '<button class="btn btn-info btn-xs" type="button" onclick="download(\''+row.fileId+'\')"><i class="fa fa-arrows"></i>&nbsp;下载</button>';
                            return operateHtml;
                        }
                    }]
                });
            ...
            function search() {
                var params = {"targetFileName":$("#targetFileName").val()};
            ...
            function view(id){
                /**layer.open({
                    ...
                    shade: false,
                    area: ['900px', '600px'],
                    content: '${ctx}/api/v1/docUploadFile/viewFile?id='  + id,
                    end: function(index){
                        $('#etaxListTable').bootstrapTable("refresh");
                    }
                });**/
             ...
             function add(){
                layer.open({
                    ...
                    shade: false,
                    area: ['800px', '600px'],
                    content: '${ctx}/api/v1/etaxExcelFile/addView',
                    end: function(index){
                        $('#etaxListTable').bootstrapTable("refresh");
                    }
                });
            ...
            function download(id){
                window.open("${ctx}/api/v1/docUploadFile/dowloadDoc?fileId=" + id);
            }
            function del(id){
